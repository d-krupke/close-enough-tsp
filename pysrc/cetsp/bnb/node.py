...
class Node:
    ...
    def branch(
        self,
        circle: Circle,
        condition: typing.Optional[typing.Callable[[PartialSolution], bool]] = None,
    ):
        assert not self.is_feasible()
        print(f"Branch {self} on {circle}.")
        if (
            self.origin is not None
            and self.origin[0].get_lower_bound() < self.get_lower_bound()
        ):
            # self.partial_solution.simplify()
            pass
        partial_solutions = [
            self.partial_solution.insert(i, circle)
            for i in range(len(self.partial_solution))
        ]
        self.branches = [
            Node(ps, (self, circle))
            for ps in partial_solutions
            if condition is None or condition(ps)
        ]
